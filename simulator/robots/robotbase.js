...
export default class Robot {
  ...
  init(model) {
    ...
    this.tcp.object.quaternion.multiply(
      new Quaternion().setFromEuler(new Euler().set(...this.tcp.rotation))
    );
    ...
    for (let joint of this.hand.movable) {
      joint.states = {
        closed: this.hand.invertOpenClose
          ? joint.limit.upper
          : joint.limit.lower,
        opened: this.hand.invertOpenClose
          ? joint.limit.lower
          : joint.limit.upper,
      };
    }
  ...
  setPose(pose) {
    if (Array.isArray(pose)) {
      if (pose.length !== this.arm.movable.length) {
        throw new Error(
          "Array length must be equal to the number of movable joints"
        );
      }
