...
import { updatePhysics, isWorldActive } from "./physics";
...
class TheSimulation {
  ...
  async run(command, ...args) {
    ...
    try {
      ...
      console.error(
        "Failed to run command '" + command + "(" + args + ")':" + e
      );
      throw e;
    }
  }
  ...
  move(poseType, pose) {
    ...
    switch (poseType) {
      case "TaskspacePose":
        ...
        const solution = this.ik.solve(ikTarget, robot, robot.ikEnabled, {
    ...
    const duration = getDuration(
      robot,
      target,
      this.velocities.move * robot.maxSpeed.move
    );
